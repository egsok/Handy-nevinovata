use crate::audio_toolkit::{
    ...
    normalize_transcription_output, remove_filler_words, remove_hallucinated_sentences,
    remove_repeated_sentences, OutputLanguageEvidence,
};
...
impl TranscriptionManager {
    ...
    pub fn finalize_stream(&self) -> Result<Option<String>> {
        ...
        // Streaming engines are non-whisper, so skip whisper hallucination cleanup.
...
fn post_process_transcription_text(
    ...
    fail_open_text_transform(raw, |raw| {
        ...
        // on Cyrillic and Cyrillic↔Latin output. Keep this repair independent
        // of the filler-removal toggle.
        ...
        let output_language = match output_language {
            ...
            {
                match detect_output_language(&unglued, supported_languages) {
                    Some(language) => {
        ...
        // and remain enabled when filler removal is disabled.
        if whisper_hallucination_cleanup {
            remove_hallucinated_sentences(&remove_repeated_sentences(&normalized))
        } else {
            normalized
        }
    })
...
mod tests {
    ...
    #[test]
    fn filler_toggle_does_not_disable_breeze_or_whisper_repairs() {
        for enabled in [false, true] {
            let settings = AppSettings {
                selected_model: "breeze-asr".to_string(),
                filler_word_removal_enabled: enabled,
                custom_filler_words: Some(vec!["uhm".to_string()]),
                ..Default::default()
            };
            let result = post_process_transcription_text(
                "uhm Привет.Мир. Покажу. Покажу. Покажу. Спасибо за просмотр.".to_string(),
                &settings,
                true,
                true,
            ...
            let expected = if enabled {
                ...
                "uhm Привет. Мир. Покажу."
            };
            assert_eq!(result, expected, "filler removal enabled={enabled}");
        }
    }

    #[test]
    fn non_whisper_output_keeps_sentences_targeted_by_whisper_cleanup() {
        let raw = "Покажу. Покажу. Покажу. Спасибо за просмотр.";
        for enabled in [false, true] {
            let settings = AppSettings {
            ...
            assert_eq!(result, raw, "filler removal enabled={enabled}");
        }
    }

    #[test]
    fn disabled_filler_removal_still_normalizes_whitespace_and_stutters() {
        let settings = AppSettings {
        ...
        let result = post_process_transcription_text(
            "  uhm  I I I think this works  ".to_string(),
            &settings,
            false,
            false,
            &OutputLanguageEvidence::Unknown,
        ...
        assert_eq!(result, "uhm I think this works");
    }

    #[test]
    fn whisper_cleanup_preserves_standalone_thanks() {
        let result = post_process_transcription_text(
            "Спасибо.".to_string(),
            &AppSettings::default(),
            true,
