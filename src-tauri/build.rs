fn main() {
    ...
    // `/usr/lib/klava-nevinovata` (the app-private resource dir; keeps these
    // libs out of the ldconfig-scanned `/usr/lib`, issue #1639) while
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("linux") {
        println!("cargo:rustc-link-arg=-Wl,-rpath,$ORIGIN/../lib/klava-nevinovata:$ORIGIN/../lib");
    }
...
/// from the exe dir); Linux deb/rpm map it into the app-private `/usr/lib/klava-nevinovata`
