...
- **Защита от галлюцинаций Whisper.** Декодер переносит не больше 128 токенов предыдущего контекста, серии одинаковых предложений схлопываются, а известные субтитровые галлюцинации вроде «Продолжение следует...» удаляются только при полном совпадении предложения. Настоящая речь с теми же словами сохраняется.
- **Свой transcription prompt с русскими примерами.** В Settings → Advanced есть initial prompt для каждого языка Whisper-моделей. Русские значения по умолчанию помогают с именами, терминами и пунктуацией.
- **Пунктуация в длинных диктовках.** `condition_on_prev_tokens=true` сохраняет контекст между 30-секундными окнами whisper.cpp, поэтому после первого окна пунктуация не пропадает.
- **Исправление границ кириллических слов в Breeze ASR.** Детерминированный post-process разделяет склеенные Breeze кириллические и смешанные Cyrillic/Latin слова, не ломая распространённые сокращения вроде `.NET` и `PDF`.
...
- **Watchdog хоткеев и высокий приоритет Windows-hook.** Форк считает ресинхронизации модификаторов, следит за зависаниями пайплайна хоткеев и запускает low-level keyboard hook с time-critical приоритетом. Официальный `handy-keys` 0.3.3 уже сверяет живое состояние модификаторов; в нашем форке остались дополнительная диагностика и повышенный приоритет потока.
...
Если потребуется, используй `sudo xattr -cr /Applications/klava-nevinovata.app`. Приложению также нужны разрешения на микрофон и Универсальный доступ в **Настройки системы → Конфиденциальность и безопасность**. Если после обновления Универсальный доступ не срабатывает, удали старую запись, добавь приложение заново и перезапусти его.
