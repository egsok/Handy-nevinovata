...
Все изменения в этом разделе напрямую улучшают результат распознавания: каждое исправляет типичную ошибку конкретной модели.
...
- **Custom transcription prompt возвращает пунктуацию в Whisper Turbo.** Turbo часто превращает русскую диктовку в сплошной текст почти без знаков препинания. Русский prompt по умолчанию устраняет эту проблему: возвращает границы предложений и пунктуацию. В Settings → Advanced его можно дополнить своими именами и терминами, чтобы модель лучше их распознавала.
